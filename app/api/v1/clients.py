...
from app.schemas.clients import MergeIn
...
def merge_client(
    ...
    # Session.execute по типам отдаёт общий Result, а rowcount есть только у
    # курсорного; для UPDATE это всегда он
    result = cast("CursorResult[Any]", db.execute(
        update(Order).where(Order.client_id == source.id).values(client_id=target.id)
    ))

...
def delete_device(
    ...
    Если для какого-то профиля это единственный разрешённый компьютер —
    отказываем: иначе профиль остался бы в режиме «только с выбранных» с
    пустым списком, и войти в него было бы нельзя ниоткуда. Новый ключ тут
    не поможет — это будет новое устройство, которого в списке тоже нет.
    """
    device = db.get(Device, device_id)
    if device is None:
        raise HTTPException(404, "Устройство не найдено")
    ...
    stranded = [
        e.name for e in db.scalars(select(Employee)).all()
        if e.access_mode == "devices" and (e.allowed_devices or []) == [device_id]
    ]
    if stranded:
        raise HTTPException(
            409,
            "Это единственный компьютер для профилей: " + ", ".join(stranded)
            + ". Сначала добавьте им другой компьютер или разрешите вход с любого.",
        )

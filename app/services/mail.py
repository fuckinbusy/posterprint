...
def decode(value: str | None) -> str:
    ...
    except Exception:  # кривой заголовок не должен ронять список
        return value.strip()
...
class _Text(HTMLParser):
    ...
    BLOCKS: ClassVar[set[str]] = {"p", "div", "br", "li", "tr", "h1", "h2", "h3", "h4", "blockquote", "table"}
...
def sanitize_html(raw: str, cid_map: dict[str, str] | None = None) -> str:
    ...
    try:
        parser.feed(raw)
        parser.close()
    except Exception:  # сломанную разметку показываем текстом
        return ""
...
def html_body(msg: EmailMessage) -> str:
...
def body_text(msg: EmailMessage) -> tuple[str, bool]:
    ...
    for part in msg.walk():
        ...
        except Exception:  # неизвестная кодировка
            payload = payload_bytes(part)
...
def payload_bytes(part: Message) -> bytes:
    """Тело части письма байтами. get_payload(decode=True) по типам может
    вернуть и вложенное письмо, и строку — нам нужны только байты."""
...
class Mailbox:
    ...
    def page(self, cfg: MailConfig, before: int | None, after: int | None, limit: int) -> dict:
        ...
        has_older = bool(chosen) and any(u < chosen[0] for u in all_uids)
        has_newer = bool(chosen) and any(u > chosen[-1] for u in all_uids)
        return {
            "messages": messages,
            "has_older": has_older,
            "has_newer": has_newer,
    ...
    # ------------------------------------------------ письмо
    def raw(self, cfg: MailConfig, uid: int, folder: str = INBOX) -> EmailMessage:
        def run(conn):
            def fetch():

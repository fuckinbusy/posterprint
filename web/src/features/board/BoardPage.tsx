...
export function BoardPage({ filter, sort }: BoardPageProps) {
  ...
  const openMenu = (order: Order, x: number, y: number) =>
    setMenu({ order, x, y });
  ...
  const edgeClass = (side: "left" | "right") =>
    ["board-edge", side, edges[side] ? "can" : "", moving === side ? "on" : ""]
      .filter(Boolean)
      .join(" ");
        ...
        >
          {statuses.statuses.map((status) => (
            <Column
              ...
              orders={sortOrders(
                (orders.data ?? []).filter(
                  (o) => o.status === status.key && matchesFocus(o, focus),
                ),
                sort,
              )}
          ...
          {menu && (
            <OrderContextMenu state={menu} onClose={() => setMenu(null)} />
          )}
        </main>
...
function Column({
  ...
  const sum =
    status.key === "cancelled"
      ? 0
      : orders.reduce((acc, o) => acc + (o.price || 0), 0);
